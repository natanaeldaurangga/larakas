<div class="modal fade" id="form-pembayaran-utang" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="/utang" class="form-create-utang" method="POST">
                    ...
                    @method('put')
                    @csrf
                    <input type="hidden" class="input-hidden" name="id_utang" value="">
                    <div class="card-body">
...
<script>
    $(document).ready(function() {
        // TODO: untuk yang utang si validasinya masih pake yang cek saldo piutang, jadi harus diganti
        $('#saldo').on('input', function() {
            $("input[name='saldo']").val($(this).inputmask('unmaskedvalue'));
        });

        $('.form-create-utang').submit(function(e) {
            e.preventDefault();
            let thisBtn = $(".ajx-simpan");
            let id_utang = $('input[name="id_utang"]').val();
            $.ajax({
                type: "put",
                url: `{{ url('/pembayaran-utang/') }}/${id_utang}`,
                data: $(this).serialize(),
                dataType: "json",
                beforeSend: function() {
                ...
                success: function(response) {
                    ...
                    $('#form-pembayaran-utang .form-control').val('');
                    $('.input-hidden').val('');
                    $('#form-pembayaran-utang').modal('hide');
                },
                complete: function() {
